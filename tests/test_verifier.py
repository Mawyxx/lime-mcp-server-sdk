...
def test_token_verifier_expired(rsa_keypair: tuple) -> None:
    ...
    verifier = TokenVerifier(expected_domain=_PIN, base_url=base, leeway_seconds=0)
    result = verifier.verify(token)
...
def test_token_verifier_invalid_token() -> None:
    ...
    verifier = TokenVerifier(expected_domain=_PIN, base_url=base)
...
def test_token_verifier_refresh_and_invalidate(rsa_keypair: tuple) -> None:
    ...
    verifier = TokenVerifier(expected_domain=_PIN, base_url=base, min_refresh_seconds=0)
...
def test_token_verifier_invalid_audience(rsa_keypair: tuple) -> None:
    ...
    verifier = TokenVerifier(expected_domain=_PIN, base_url=base, audience="mcp")
    result = verifier.verify(token)
    assert result.is_valid is False
...
@respx.mock
def test_token_verifier_missing_domain_claim(rsa_keypair: tuple) -> None:
    private_key, jwk = rsa_keypair
    base = "https://lime.pics"
    respx.get(f"{base}{METADATA_PATH}").respond(json=_metadata_body())
    respx.get(f"{base}{JWKS_PATH}").respond(json=_jwks_body(jwk))
    ...
    result = verifier.verify(token)
    assert result.is_valid is False
    assert result.error == "Missing domain claim"
    verifier.close()


@respx.mock
def test_token_verifier_domain_mismatch(rsa_keypair: tuple) -> None:
    private_key, jwk = rsa_keypair
    base = "https://lime.pics"
    respx.get(f"{base}{METADATA_PATH}").respond(json=_metadata_body())
    respx.get(f"{base}{JWKS_PATH}").respond(json=_jwks_body(jwk))
...
@respx.mock
def test_token_verifier_invalid_domain_claim(rsa_keypair: tuple) -> None:
    private_key, jwk = rsa_keypair
    base = "https://lime.pics"
    respx.get(f"{base}{METADATA_PATH}").respond(json=_metadata_body())
    respx.get(f"{base}{JWKS_PATH}").respond(json=_jwks_body(jwk))

    token = sign_mcp_token(private_key, domain="https://rs.example/path")
    verifier = TokenVerifier(expected_domain=_PIN, base_url=base)
    result = verifier.verify(token)
    assert result.is_valid is False
    assert result.error == "Invalid domain claim"
    verifier.close()
...
def test_token_verifier_network_error_no_cache() -> None:
    ...
    config = LimeConfig(base_url=base, expected_domain=_PIN)
...
def test_token_verifier_requires_expected_domain(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("LIME_EXPECTED_DOMAIN", raising=False)
    with pytest.raises(ValueError, match="expected_domain is required"):
        TokenVerifier(base_url="https://lime.pics")


def test_token_verifier_port_pin_rejected() -> None:
    with pytest.raises(ValueError, match="domain must not include a port"):
        TokenVerifier(expected_domain="rs.example:443", base_url="https://lime.pics")


@respx.mock
def test_token_verifier_env_only_pin(
...
def test_token_verifier_valid_claims_property(rsa_keypair: tuple) -> None:
    ...
    assert result.valid_claims["domain"] == _PIN
    verifier.close()
...
def test_token_verifier_warmup_success(rsa_keypair: tuple) -> None:
    ...
    config = LimeConfig(base_url=base, min_refresh_seconds=0, expected_domain=_PIN)
    cache = JwksCache(config)
    verifier = TokenVerifier(config=config, cache=cache)
